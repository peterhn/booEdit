...
namespace
{
    ...
    EditorAreas determineEditorAreas(int lineCount)
    {
        ...
        int lineNumberLength = intToString(lineCount).length();
    ...
    void redrawLineNumberArea(const Area& lineNumberArea, const Editor& editor, int topVisibleLine)
    {
        ...
        for (int y = lineNumberArea.tlxy.y; y <= lineNumberArea.brxy.y; ++y)
        {
            ...
            {
                std::string lineNumber = intToString(currentLine);
        ...
        const std::string& currentErrorMessage)
    {
        ...
        {
            mvaddch(statusBarArea.tlxy.y, x, '-');
            mvaddch(statusBarArea.brxy.y, x, ' ');
        }
        ...
        mvaddstr(statusBarArea.brxy.y, statusBarArea.brxy.x - posStr.length() + 1, posStr.c_str());

        if (currentErrorMessage.length() > 0)
        {
            int maxErrorMessageWidth =
                (statusBarArea.brxy.x - statusBarArea.tlxy.x + 1) - (posStr.length() + 3);
